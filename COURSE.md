...
Generated from [`lessons/`](lessons/) on 2026-09-20.
...
few-shot demonstration of _how this conversation goes_, and the model imitates its
...
That is also why the rule is a _prefix_ rule. A KV state is only valid for the
...
_"For the rest of the session"_ overstates it. A **one-off** edit costs one
...
tool search sidesteps the problem entirely by _appending_ schemas rather than
...
One workspace, one job, five beats. The job: _"free up some space in this
workspace."_ The workspace:
...
And in the system prompt, the sentence every team writes: _"Never delete files
without asking."_
...
rule, but the fact that the rule now sees the _argument_ rather than a sentence
about the argument.
